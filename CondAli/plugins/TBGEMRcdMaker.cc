...
TBGEMRcdMaker::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for (auto roll : gemGeo->etaPartitions()) {
	...
	if (ieta%2 == 1){
	  ...
	  phir = HepRotation(Hep3Vector(cos(pShift2),0.,-sin(pShift2)).unit(),
			     Hep3Vector(          0.,1.,           0.).unit(),
			     Hep3Vector(sin(pShift2),0., cos(pShift2)).unit());
	}else{
	...
	phir = HepRotation(Hep3Vector(cos(pShift3),0.,-sin(pShift3)).unit(),
			   Hep3Vector(          0.,1.,           0.).unit(),
			   Hep3Vector(sin(pShift3),0., cos(pShift3)).unit());
	auto totr = phir*hrot;
	euler = totr.inverse().eulerAngles();
      }else if (idx == 4){
	if (ieta%2 == 1){
	...
	phir = HepRotation(Hep3Vector(cos(pShift4),0.,-sin(pShift4)).unit(),
			   Hep3Vector(          0.,1.,           0.).unit(),
			   Hep3Vector(sin(pShift4),0., cos(pShift4)).unit());
	auto totr = phir*hrot;
        euler = totr.inverse().eulerAngles();
