...
void GTRTrakExtrapoler::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if (prout)
    std::cout <<" GEM Tracker Chamber "<<chname<<std::endl;
  ...
  if (prout) 
    std::cout <<chname<<" == Tracker GEM selected "<<TrCh->id()<<std::endl;
  ...
  if (prout)
    std::cout <<chname<<" Propagation to the Tracker Chamber starting from each measurement"<<std::endl;
  ...
  for (std::vector<TrajectoryMeasurement>::iterator ms=meass.begin();ms!=meass.end();ms++){
    if (prout)
      std::cout <<chname<<" == Measurement "<<ms->updatedState().globalPosition()<<" Rechit " <<ms->recHit()->globalPosition()<<std::endl;       
  ...
  if ( bp.bounds().inside(lpm,elo,100.)){
    if (prout)
      std::cout <<chname<<" Local P "<<lp<<" "<<lpm<<" Error on x "<<sqrt(elo.xx())<<" Error on y "<<sqrt(elo.yy())<<" inside ? "<<bp.bounds().inside(lpm,elo,100.)<<std::endl;
    ...
    for (auto ix = rh.first; ix != rh.second; ++ix){
      if (prout)
	std::cout <<chname<<" RECHITS "<<ix->localPosition()<<std::endl;
      nrh+=1;
    }
    if (prout)
      std::cout <<chname<< " +++ Number of Rechits "<<nrh<<std::endl;
  }
